...
class PrunedLlamaMLPFunction(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        col_norm_gate = gate_2d.norm(p=2, dim=1)  # (I,)
        col_norm_up   = up_2d.norm(p=2, dim=1)    # (I,)
        ...
        gate_small = torch.index_select(gate, dim=1, index=top_indices)
        up_small   = torch.index_select(up,   dim=1, index=top_indices)
        a_small    = torch.index_select(a,    dim=1, index=top_indices)
    ...
    def backward(ctx, grad_out):
        ...
        grad_c_small = torch.index_select(grad_c, dim=1, index=top_indices)   # shape (B,S,keep_num)
        ...
        grad_gate.index_copy_(1, top_indices, grad_gate_small)
        grad_up.index_copy_(1, top_indices, grad_up_small)
